<template>
    ...
    <AuthenticatedLayout>
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Status</span>
                </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="border-b border-gray-200 pb-5 sm:flex sm:items-center sm:justify-between">
            ...
            <h3 class="text-lg leading-6 text-gray-900 mt-4">Auxiliares: Lista e cadastro de Status</h3>
    ...
    <div class="row">
        <div class="inline-block min-w-full overflow-w-auto rounded-lg shadow bg-white">
            ...
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr class="border-b bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">Descrição</p>
                        </td>                       
                ...
                <tbody>
                    <tr v-for="status, i in statuses.data" :key="status.id" class="text-gray-700">
                        <td class="border-b border-gray-200 bg-white px-4 py-4 text-sm">{{ (i+1) }}</td>
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ status.status_name }}</p>
                        </td>  
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <p class="text-gray-900 whitespace-no-wrap">{{ status.description }}</p>
                        </td>                     
                        
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <WarningButtom @click="event => openModal(2, status.description, status.id)">
                        ...
                        <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                            <DangerButton @click="event => deleteStatus(status.id, status.status_name)">
                                <span class="flex intens-center gap-x-3">
            ...
            <Pagination class="mt-6 bg-white py-5 py-2" :links="statuses.links" />
        
            
        </div>
        <Modal :show="modal" @close="closeModal">
            <h2 class="p-3 pt-3 text-lg text-center font.medium text-hray-900">{{ titleStatus}}</h2>
            
            <div class="p-3 mt-3">
                <InputLabel for="description" value="Descrição:"></InputLabel>
                <TextInput id="description" ref="descriptionInput"
                v-model="form.description" type="text" class="mt-1 block w-3/4"
...
const openModal = (op, description, status) =>{
    modal.value = true;
    operation.value = op;
    id.value = status;
    if(op == 1){
        titleStatus.value = 'Novo Status';
    }else{
        titleStatus.value = 'Editar Status';        
...
const save = () => {
    if(operation.value == 1){
        form.post(route('admin.cadastros.statuses.store'),{
            onSuccess: () => {message('Status statuses com sucesso!')}
        });
    }else{
        form.put(route('admin.cadastros.statuses.update',id.value),{
            onSuccess: () => {message('Status atualizada com sucesso!')}
        });
    }
}
...
const deleteStatus= (id,status_name) =>{
    const alerta = Swal.mixin({
        buttonsStyling:true
    });

    alerta.fire({
        title:'Você realmente quer deletar '+status_name+' ?',
        ...
        if(result.isConfirmed){
            form.delete(route('admin.cadastros.statuses.destroy', id))
        }
    });
}
